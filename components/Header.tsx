...
const Header: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between px-6 py-4 md:px-12 bg-white/95 backdrop-blur-md shadow-sm transition-all">
      ...
      <nav className="hidden md:block">
        <ul className="flex gap-8 items-center">
          ...
          <li className="relative group">
            <button className="flex items-center gap-1 text-sm font-bold text-gray-600 uppercase transition-colors hover:text-[#01A982] py-2">
            ...
            <div className="absolute top-full left-0 w-48 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-2 group-hover:translate-y-0">
               <div className="bg-white shadow-xl rounded-xl border border-gray-100 overflow-hidden">
                  <a href="#business-direction" className="block px-5 py-3 text-sm font-medium text-gray-600 hover:text-[#01A982] hover:bg-green-50 border-b border-gray-50">
                      사업방향
                  </a>
                  <a href="#organization" className="block px-5 py-3 text-sm font-medium text-gray-600 hover:text-[#01A982] hover:bg-green-50 border-b border-gray-50">
                      조직도
                  </a>
                  <a href="#directions" className="block px-5 py-3 text-sm font-medium text-gray-600 hover:text-[#01A982] hover:bg-green-50">
                      오시는 길
                  </a>
               </div>
            </div>
               ...
               <div className="bg-white shadow-xl rounded-xl border border-gray-100 overflow-hidden">
                  {productLinks.map((link) => (
                    <a 
                      ...
                      className="block px-5 py-3 text-sm font-medium text-gray-600 hover:text-[#01A982] hover:bg-green-50 border-b border-gray-50 last:border-0"
                    >
                      {link.name}
                    </a>
                  ))}
      ...
      {/* Mobile Menu Button */}
      <button
        className="md:hidden text-gray-700 hover:text-[#01A982]"
        onClick={toggleMenu}
          ...
          <ul className="flex flex-col p-4">
            ...
            <li className="border-b border-gray-100">
              <button 
                ...
                className="w-full flex items-center justify-between py-4 text-sm font-bold text-gray-600 uppercase hover:text-[#01A982]"
              >
                Home
              ...
              {isHomeSubmenuOpen && (
                <ul className="bg-gray-50 rounded-lg mb-4 overflow-hidden">
                   <li>
                      <a href="#business-direction" onClick={closeMenu} className="block px-6 py-3 text-sm text-gray-500 hover:text-[#01A982]">
                        사업방향
                      </a>
                   </li>
                   <li>
                      <a href="#organization" onClick={closeMenu} className="block px-6 py-3 text-sm text-gray-500 hover:text-[#01A982]">
                        조직도
                      </a>
                   </li>
                   <li>
                      <a href="#directions" onClick={closeMenu} className="block px-6 py-3 text-sm text-gray-500 hover:text-[#01A982]">
                        오시는 길
                      </a>
                   </li>
                </ul>
                   ...
                   {productLinks.map((link) => (
                      <li key={link.name}>
                        <a 
                          ...
                          rel="noopener noreferrer" 
                          onClick={closeMenu} 
                          className="block px-6 py-3 text-sm text-gray-500 hover:text-[#01A982]"
                        >
                          {link.name}
                        </a>
                      </li>
